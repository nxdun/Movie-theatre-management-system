...
function UpdateMovieShedularPage(props) {
  ...
  const onCancelcnBtnClick = () => {
    navigate("/sheduleMovie"); // Navigate main page
  };
  ...
  const onFinish = (values) => {
    ...
    axios
      .put(`/admin/updateMovieShedularDetails/${sheduleId}`, formData)
      .then((res) => {
  ...
  // New state variable to hold the selected schedule
  ...
  const getallMovieShedularDetails = () => {
    axios
      ...
      .then((res) => {
        ...
        const filteredMovie = allMovieShedulars.find(
          (item) => item._id === sheduleId
        );
        console.log("ssssss", filteredMovie);
        if (filteredMovie) {
          // Set the selected schedule
          setSelectedSchedule(filteredMovie);
          // 'filteredMovie' contains the data object with the matching '_id'
          ...
          setSelectMovie({
            label: filteredMovie.MovieName,
            value: filteredMovie.MovieId,
          });
